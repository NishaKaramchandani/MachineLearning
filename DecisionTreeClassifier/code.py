...
for i in range(len(Xs)):
    X_train, X_test, y_train, y_test = train_test_split(Xs[i], ys[i], test_size=1. / 3, random_state=144)
    for d in range(15):
        print("Depth::",d)
        desicion_tree = DecisionTreeClassifier(criterion="entropy", max_depth=d + 1, random_state=435)
        decision_tree = desicion_tree.fit(X_train, y_train)
        y_train_predict = decision_tree.predict(X_train)
        train_ac[i][d] = accuracy_score(y_train, y_train_predict)
        y_test_predict = decision_tree.predict(X_test)
        test_ac[i][d] = accuracy_score(y_test,y_test_predict)
